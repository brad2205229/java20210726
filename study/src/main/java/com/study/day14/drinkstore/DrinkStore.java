...
public class DrinkStore {
	...
	public static void main(String[] args) {
		...
		// 是問我要買 10 杯 milkTea + 15 杯 latte 共要花多少錢
		...
		drinks.add(milkTea);
		drinks.add(latte);
		int price = drinks.stream().mapToInt(drink -> drink.getAmount() * 
				drink.getProducts().stream().mapToInt(Product::getPrice).sum())
				.sum();
		System.out.println(price);
		
		// 請問此筆訂單商家賺多少錢
		int cost = drinks.stream().mapToInt(drink -> drink.getAmount() * 
				drink.getProducts().stream().mapToInt(Product::getCost).sum())
				.sum();
		System.out.printf("成本: $%d, 業績: $%d, 獲利: $%d\n", cost, price, (price-cost));
	}
